...
export class ProductDetailsComponent implements OnInit {
    ...
    this.router.events
      ...
      .subscribe(event => {
        this.titleService.setTitle(
          this.titleCaseWord(event["snapshot"].params["id"]) +
            " " +
  ...
  checkProductQuantity(qantity: Number) {
    if (qantity === 0) {
      this.statusTxt = {
        type: this.constants.prod_NA,
        text: "Product Unavailable"
      };
    } else if (qantity < 5) {
      this.statusTxt = {
        type: this.constants.prod_FEW,
        text: "Hurry, Only " + qantity + " left!"
      };
    } else {
      this.statusTxt = {
        type: this.constants.prod_AVAILABLE,
        text: "Available"
      };
    }
  ...
  addtocart() {
    ...
    this.apiService.addToCart(sessionInfo, this.productDt).subscribe(
      data => {
    ...
    this.apiService.isAvailableInCart(email).subscribe(
      data => {
        ...
        if (
          ...
          data.productsInCart[0].cartResponse.length > 0
        ) {
    ...
    this.apiService.getErrorMessage().then(
      res => {
        if (this.sharedService.errorObj.length === 0) {
          this.sharedService.errorObj =
            res["srverrors"] && res["srverrors"][0]
              ? res["srverrors"][0]["errorslist"]
              : [];
