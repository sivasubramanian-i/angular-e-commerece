...
export class ProductListComponent implements OnInit, OnChanges, OnDestroy {
  ...
  async getProductLists() {
    try {
      ...
      await this.route.params.subscribe(params => {
        ...
        this.productObsv = this.apiService.getProducts().subscribe(
          data => {
            console.log(data);
            this.products = data;
  ...
  sortbyMessage(event): void {
    ...
    const SortBy = (x, y) => {
      if (this.option === this.constants.sortbyPrice_L2H) {
        ...
        return x.DateOfEntry === y.DateOfEntry
          ? 0
          : x.DateOfEntry > y.DateOfEntry
          ? 1
          : -1;
      } else {
        return x.DateOfEntry === y.DateOfEntry
  ...
  filterBySubcat(subcat) {
    ...
    if (subcat !== undefined) {
      const productsByCat = this.filteredProducts.filter(
        elemt => elemt.SubCategory === subcat
      );
