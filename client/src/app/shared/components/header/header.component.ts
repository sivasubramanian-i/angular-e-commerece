    ...
    try {
      ...
      await this.sharedService.content.subscribe(res => {
        ...
        if (this.custStatus === false) {
          ...
          this.content = siteObj.filter(
            elemt => elemt.pageHeader !== pageHeader
          );
        } else {
  ...
  */
  ...
  modifyLogoutLink(i, ct) {
    ...
    this.renderer.setProperty(
      link,
      "innerHTML",
      '<a href="javascript:void(0)" class="nav-link">' + ct.pageName + "</a>"
    );
